    ...
    switch (opts.category) {
      case "cluster":
        switch (queryName) {
          ...
          case "gpuRequests":
            return `sum(kube_pod_container_resource_requests{node=~"${opts.nodes}", resource="nvidia_com_gpu"} * on(pod, namespace) group_left() (kube_pod_status_phase{phase="Running"} == 1))`;
          case "fsSize":
